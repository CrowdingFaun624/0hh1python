...
def generate_solution(size:tuple[int,int], seed:int=None, colors:int=2) -> list[int]:
    ...
    def clear_row_from_tiles(y:int) -> None:
        ...
        # output_rows[y] = None
        if wave_collapse_storage[y] is not None: # if the line already exists, remove it and put it back in valid rows. # TODO: remove the in check
    ...
    def get_valid_rows() -> list[str]:
        ...
        already_indexes:set[int] = set()
        random_index = list(range(0, colors ** size[0]))
        random.shuffle(random_index)
        for index in random_index:
            if index in already_indexes: continue
            already_indexes.add(index)
    ...
    def generate_initial_valid_rows() -> list[str]:
        approximate_maximum = int(0.93 * 1.17963 * exp(0.442493 * size[0]))
        desired_maximum = size[0] * size[1] * colors * 1
        generation_amount = GENERATION_AMOUNT # min(approximate_maximum, desired_maximum) - 1
        if generation_amount == approximate_maximum: raise ValueError("Size is too small to trigger effective (%s vs. %s)" % (approximate_maximum, desired_maximum))
        # print("Using %s out of (approximate maximum: %s, desired maximum: %s)" % (generation_amount, approximate_maximum, desired_maximum))
        return [next(generator) for i in range(generation_amount)]
    def generate_more_valid_rows(amount:int) -> list[str]:
        return [next(generator) for i in range(amount)]
    ...
    generator = get_valid_rows()
    valid_rows = generate_initial_valid_rows()
    random.shuffle(valid_rows)

    y_position = 0
    tiles:list[int] = [0 for i in range(size[0] * size[1])] # output
    wave_collapse_storage:list[str] = [None] * size[1] # TODO: initialize this as a list with size[1] None items
    ...
    while y_position < size[1]:
        i += 1
        # if i % 100000 == 0: LevelPrinter.print_board(tiles, size)
        ...
        if grid_is_valid(y_position == size[1] - 1):
            ...
            # if row_tries[y_position] >= len(valid_rows):
                ...
                if this_state in previous_states: random.shuffle(valid_rows)# ; print("Shuffle!", shuffle_count)
...
if __name__ == "__main__":
    ...
    size = 18
    for i in range(994,3524,256):
        GENERATION_AMOUNT = i
        # full, empty, other_data = cProfile.run("generate(size, 1234, 2)")
        elapsed_times:list[float] = [generate(size, i, colors=2) for i in range(1)]
        print("%s," % str(GENERATION_AMOUNT).zfill(4), mean(elapsed_times),sep="")
