...
class SwitchButton(Button.Button):
    ...
    def get_new_surface(self, current_time:float) -> pygame.Surface:
        ...
        pygame.draw.rect(surface, (255, 255, 255), pygame.Rect(OUTER_RADIUS - INNER_BORDER_RADIUS, HEIGHT / 2 - INNER_RADIUS, WIDTH - (OUTER_RADIUS * 2) + (2 * INNER_BORDER_RADIUS), INNER_RADIUS * 2), INNER_BORDER_RADIUS)
        pygame.draw.circle(surface, inner_color, (OUTER_RADIUS, HEIGHT / 2), INNER_RADIUS, 0, False, True, True, False)
        pygame.draw.circle(surface, (255,255,255), (OUTER_RADIUS, HEIGHT / 2), INNER_RADIUS, INNER_BORDER_RADIUS, False, True, True, False)
        pygame.draw.circle(surface, inner_color, (WIDTH - OUTER_RADIUS, HEIGHT / 2), INNER_RADIUS, 0, True, False, False, True)
        pygame.draw.circle(surface, (255,255,255), (WIDTH - OUTER_RADIUS, HEIGHT / 2), INNER_RADIUS, INNER_BORDER_RADIUS, True, False, False, True)
        progress = self.animation.get(current_time)
        x_position = Bezier.linear_bezier(OUTER_RADIUS, WIDTH - OUTER_RADIUS, progress)
        pygame.draw.circle(surface, (128, 128, 128), (x_position, HEIGHT / 2), OUTER_RADIUS)
        pygame.draw.circle(surface, (255, 255, 255), (x_position, HEIGHT / 2), OUTER_RADIUS, OUTER_BORDER_RADIUS)
        return surface
