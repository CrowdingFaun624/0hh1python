...
def enter_board_from_level_selector(level_selector:LevelSelector.LevelSelector) -> list[tuple[Drawable.Drawable,int]]:
    ...
    return [(LoadingScreen.LoadingScreen(board, (board_size, board_size), finish_loading_screen, position=((level_selector.display_size[0] - board_size) / 2, (level_selector.display_size[1] - board_size) / 2)), 1)]

def finish_loading_screen(loading_screen:LoadingScreen.LoadingScreen) -> list[tuple[Drawable.Drawable,int]]:
