...
class ButtonPanel(Enablable.Enablable):
    ...
    def __init__(self, button_parameters:list[tuple[str|tuple[pygame.Surface,tuple[Callable[[],pygame.Surface]],list,dict],tuple[Callable[[],None|list[tuple[Drawable.Drawable,int]]],list,dict]]]) -> None:
    ...
    def calculate_positions(self) -> None:
        ...
        sum_of_button_width = sum((button.get_surface().get_size()[0] for button in self.children))
        spacing = (horizontal_space - sum_of_button_width) / (len(self.children) + 1)
        x = self.left_constraint
        for button in self.children:
            y = int((self.top_constraint + self.bottom_constraint) / 2 - button.get_surface().get_size()[1] / 2)
            x += spacing
            button.position = (float(x), float(y))
            x += button.get_surface().get_size()[0]
