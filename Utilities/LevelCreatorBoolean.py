...
def count_unknown_tiles(tiles:list[list[bool]]) -> int:
    '''Returns the number of tiles whose values are not completely known.'''
    return [tile.count(True) == 1 for tile in tiles].count(False)
...
def restore_cache(tiles_values:list[list[bool]], tiles_cache:list[list[bool]], colors:int) -> None:
    DEFAULT = [True] * colors
    for index, tile in enumerate(tiles_cache):
        if tile != DEFAULT: tiles_values[index] = tile[:]

def solve(size:tuple[int,int], tiles_values:list[list[bool]], current_tile_index:int, dependencies:list[list[int]], tiles_cache:list[list[bool]], colors:int) -> bool:
    ...
    for total_tries in range(size[0] * size[1] * 50): # it can loop around again and continue trying to solve if it fails the first time.
        ...
        for tile_index in tile_order: # the index of this resets every time it finds a tile's value, since it breaks.
            if tiles_values[tile_index].count(True) != 1 and previous_tile != tile_index:
        ...
        elif tiles_values[current_tile_index].count(True) == 1: return True # if the desired tile has been solved
    else: raise RuntimeError("The board expired!")
    # NOTE: before it returns, it also activates a function `v()`, which does nothing. It could potentially be reassigned somewhere
    return [tile.count(True) == 1 for tile in tiles_values].count(False) == 0
...
def breakdown_tile(size:tuple[int,int], tiles_values:list[list[bool]], tile_index:int, dependencies:list[list[int]], tiles_cache:list[list[bool]], colors:int=2) -> bool:
    ...
    if empty_column_pair_with_index is not None and tiles_values[tile_index].count(True) != 1 and see_if_it_has_a_clone(tiles_values, tile_index, empty_column_pair_with_index, size, dependencies, tiles_cache, colors):
        was_successful = True
    if empty_row_pair_with_index is not None and tiles_values[tile_index].count(True) != 1 and see_if_it_has_a_clone(tiles_values, tile_index, empty_row_pair_with_index, size, dependencies, tiles_cache, colors):
        was_successful = True
    return was_successful

def see_if_it_has_a_clone(tiles_values:list[list[bool]], tile_index:int, other_tile_index:int, size:tuple[int,int], dependencies:list[list[int]], tiles_cache:list[list[bool]], colors:int) -> bool:
    ...
    POSSIBLE_VALUES = list(range(colors))
    ...
    for value in POSSIBLE_VALUES:
        if not tiles_values[tile_index][value]: continue
        desired_color_tile = int_to_bools(value, colors)
        if this_row_or_column_values.count(desired_color_tile) >= max_per_row_or_column: continue
        for other_value in POSSIBLE_VALUES:
            if value == other_value: continue
            if not tiles_values[other_tile_index][other_value]: continue
            if this_row_or_column_values.count(desired_color_tile) >= max_per_row_or_column: continue
            ...
            tiles_values[tile_index] = int_to_bools(value, colors); tiles_values[other_tile_index] = int_to_bools(other_value, colors)
...
def row_or_column_is_cloning(tiles_values:list[list[bool]], tile_index:int, other_tile_index:int, size:tuple[int,int], colors:int, index_carrier:list[int]|None=None) -> bool:
...
def rule_out_value(tile:list[bool], value:int) -> None:
    # if tile[value]: tile.remove(value)
    tile[value] = False
def tile_is(tile:list[bool], value:int) -> bool:
    '''Returns if the tile is known to be only the given value'''
    if not tile[value]: return False
    return tile.count(True) == 1

def collect(size:tuple[int,int], tiles_values:list[list[bool]], tile_index:int, dependencies:list[list[int]], tiles_cache:list[list[bool]], colors:int) -> tuple[bool, int|None,int|None]:
    ...
    POSSIBLE_VALUES = list(range(colors))
    did_something = False
    for testing_value in POSSIBLE_VALUES:
        if not tiles_values[tile_index][testing_value]: continue # if there's nothing to remove, then don't remove anything
        for direction in DIRECTIONS: # caps at two in a row
            if not tiles_values[tile_index][testing_value]: continue
            tile_in_direction1 = get_tile_in_direction(tile_index, direction, size)
            if tile_in_direction1 is None or not tile_is(tiles_values[tile_in_direction1], testing_value): continue
            tile_in_direction2 = get_tile_in_direction(tile_in_direction1, direction, size)
            if tile_in_direction2 is None or not tile_is(tiles_values[tile_in_direction2], testing_value): continue
            ...
            did_something = True # TODO: break early if did something is true
        if tiles_values[tile_index][testing_value]:
            left_tile = get_tile_in_direction(tile_index, DIRECTIONS[0], size)
            if left_tile is not None and tile_is(tiles_values[left_tile], testing_value):
                right_tile = get_tile_in_direction(tile_index, DIRECTIONS[1], size)
                if right_tile is not None and tile_is(tiles_values[right_tile], testing_value):
        ...
        if tiles_values[tile_index][testing_value]:
            ...
            if up_tile is not None and tile_is(tiles_values[up_tile], testing_value):
                down_tile = get_tile_in_direction(tile_index, DIRECTIONS[2], size)
                if down_tile is not None and tile_is(tiles_values[down_tile], testing_value):
    ...
    def add_dependencies(index_list:list[int], value:int) -> list[int]:
        ...
        for index in index_list:
            if tile_is(tiles_values[index], value): dependency.append(index)
        return dependency
    def apply_dependencies(index_list:list[int], dependency:list[int], value:int) -> bool:
        ...
        was_successful = False
        for index in index_list:
            if not tiles_values[index][value] or tiles_values[index].count(True) == 1: continue # NOTE: I am having a brain fart with this and this line may be wrong. Original is if it's not empty
    ...
    row_values_counts = [row_values.count(int_to_bools(color, colors)) for color in POSSIBLE_VALUES]
    if row_values_counts.count(max_per_row) != colors: # if the row is not full
        for color in POSSIBLE_VALUES:
            if row_values_counts[color] >= max_per_row:
    ...
    if count_unknown_tiles(row_values) == 2: # this is used for cloning
        for index, row_tile_index in enumerate(row_indexes):
            if row_values[index].count(True) != 1 and index != tile_pos[0]:
    ...
    column_values_counts = [column_values.count(int_to_bools(color, colors)) for color in POSSIBLE_VALUES]
    if column_values_counts.count(max_per_column) != colors: # if the column is not full
        for color in POSSIBLE_VALUES:
            if column_values_counts[color] >= max_per_column:
    ...
    if count_unknown_tiles(column_values) == 2:
        for index, column_tile_index in enumerate(column_indexes):
            if column_values[index].count(True) != 1 and index != tile_pos[1]:
                empty_column_pair_with = column_tile_index
                break
        else: raise RuntimeError()
    return did_something, empty_row_pair_with, empty_column_pair_with

def get_values(tiles_values:list[list[bool]], indexes:list[int]) -> list[list[bool]]:
...
def strip_dependencies(dependencies:list[list[int]], tiles:list[int], tile_index:int, tiles_cache:list[list[bool]], colors:int) -> None:
    ...
    for affected_tile in affected_tiles:
        ...
        # tiles[affected_tile] = DEFAULT[:]

def copy_tiles(tiles:list[list[int]]) -> list[list[int]]:
    return [copy_tile[:] for copy_tile in tiles]
def int_to_bools(value:int, colors:int) -> list[bool]:
    '''Returns a list with at least one False and one True, len(colors). output[value] is always True.'''
    output = [False] * colors
    output[value] = True
    return output
...
def breakdown(tiles:list[int], size:tuple[int,int], seed:int, colors:int=2, quality_requirement:int|None=None) -> list[int]:
    ...
    tiles = [int_to_bools(tile - 1, colors) for tile in tiles]
    ...
    DEFAULT = [True] * colors
    tiles_cache:list[int] = [DEFAULT] * (size[0] * size[1])
...
def collapse_board(tiles:list[list[int]], colors:int=None, strict:bool=False) -> list[int]:
    output:list[int] = []
    for tile in tiles:
        if tile.count(True) == 0: raise ValueError("0-length tile!")
        if tile.count(True) == 1: output.append(tile.index(True) + 1)
        elif strict and tile.count(True) < colors: raise ValueError("%s-length tile!" % tile.count(True))
        else: output.append(0)
    return output

def print_board(tiles:list[int]|list[list[bool]]|str, size:tuple[int,int]|int) -> None:
...
if __name__ == "__main__":
    ...
    full, empty, other_data = generate(size, 1, colors=3)
...
# TODO: test if it's more efficient to do rectangular boards horizontally or vertically
